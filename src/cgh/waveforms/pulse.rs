...
impl LivePulseTrain {
    pub fn new(data: PulseData, pulse_env_duration: f64, period_env: f64, future_frame_index: usize) -> Self {
...
impl DigitalSource for LivePulseTrain {
    ...
    fn eval(&mut self, i: usize) -> bool {
        let b = i.checked_sub(self.offset_sclk).map_or(false, |i| {
            ...
            // let t = ( ((j % self.pulse_env_period_sclk) - (j % self.period_env_sclk)) / self.period_env_sclk ) % self.pulse_env_len ;
